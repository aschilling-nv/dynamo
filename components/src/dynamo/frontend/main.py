...
def parse_args():
    ...
    parser.add_argument(
        ...
        in ("true", "1", "yes", "y"),
        help=f"Enable on-demand metrics pulling from NIM backend when /metrics endpoint is called. Cannot be used with --nim-metrics-polling-interval-seconds > 0. Can be set via {NIM_METRICS_ON_DEMAND_ENV_VAR} env var (accepts 'true', '1', 'yes', or 'y', case-insensitive).",
    )
